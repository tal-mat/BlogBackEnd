...
const lastActionTimes: Map<string, number> = new Map();



let user = {} as User;

const authUserSecured = (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (token) {
            ...
            const decodedToken = jwt.verify(token, MY_SECRET as string);
            // @ts-ignore
            ...
            if (!lastActionTime || currentTime - lastActionTime >= 1 * 60 * 1000) {
                ...
                const updatedToken = jwt.sign(user, MY_SECRET as string, { expiresIn: '2m' });
